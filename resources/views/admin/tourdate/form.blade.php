        ...
        <div class="row">
            <div class="col-md-9">
                <div class="card card-primary">
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-6">
                                <div class="form-group {{ $errors->first('t_journeys') ? 'has-error' : '' }} ">
                                    <label>Hành trình <sup class="text-danger">(*)</sup></label>
                                    <div>
                     ...
                     <div class="row">
                         <div class="col-sm-12 col-md-6">
                             ...
                             <select name="t_starting_gate" id="t_starting_gate" class="form-control">
                                ...
                                <!--Thêm các option cho các điểm đến khác-->
                            </select>
                    ...
                    <div class="col-sm-12 col-md-6">
                        <div class="form-group">
                            <label for="t_destination">Điểm đến</label>
                            <select name="t_destination" id="t_destination" class="form-control">
                                <option value="">Chọn điểm đến</option>
                                <option value="Đà Nẵng" {{ old('t_destination', isset($tour) ? $tour->t_destination : '') == 'Đà Nẵng' ? 'selected' : '' }}>Đà Nẵng</option>
                                <option value="Nha Trang" {{ old('t_destination', isset($tour) ? $tour->t_destination : '') == 'Nha Trang' ? 'selected' : '' }}>Nha Trang</option>
                                <option value="Hà Nội" {{ old('t_destination', isset($tour) ? $tour->t_destination : '') == 'Hà Nội' ? 'selected' : '' }}>Hà Nội</option>
                                <option value="Phú Quốc" {{ old('t_destination', isset($tour) ? $tour->t_destination : '') == 'Phú Quốc' ? 'selected' : '' }}>Phú Quốc</option>
                                <option value="Hồ Chí Minh" {{ old('t_destination', isset($tour) ? $tour->t_destination : '') == 'Hồ Chí Minh' ? 'selected' : '' }}>Hồ Chí Minh</option>
                                <!--Thêm các option cho các điểm đến khác-->
                            </select>
                            <span class="text-danger"><p class="mg-t-5">{{ $errors->first('t_destination') }}</p></span>
                        </div>
                    </div> 
                </div>
                        ...
                        <div class="form-group {{ $errors->first('t_menu') ? 'has-error' : '' }} ">
                            <label for="inputEmail3" class="control-label default">Mô tả Thực Đơn </label>
                            <div>
                                <textarea name="t_menu" id="t_menu" cols="30" rows="10" class="form-control" style="height: 225px;">{{ old('t_menu', isset($tour) ? $tour->t_menu : '') }}</textarea>
                                <script>
                                    ckeditor(t_menu);
                                </script>
                                @if ($errors->first('t_menu'))
                                    <span class="text-danger">{{ $errors->first('t_menu') }}</span>
                                @endif
                            </div>
                        </div>
